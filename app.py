    ...
    if st.sidebar.button("Generate Forecast"):
        ...
        if show_entire_forecast:
            ...
            df_forecast = forecast[
                forecast['ds'] > future['ds'].max() - pd.Timedelta(months=input_months)
            ]
        ...
        fig.update_layout(
            title_text=f"Sales Forecast on monthly basis",
            xaxis_title="Date",
            yaxis_title="Sales",
            legend=dict(
                yanchor="top",
            ...
            rangeselector=dict(
                buttons=list([
                    dict(count=1, label="1m", step="month", stepmode="backward"),   #for 1 month
                    dict(count=3, label="3m", step="month", stepmode="backward"),  #for 3 months
                    dict(count=6, label="6m", step="month", stepmode="backward"), #for 6 months
